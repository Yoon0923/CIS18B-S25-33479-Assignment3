...
class BankAccount {
    ...
    public void deposit(double amount) throws NegativeDepositException, InvalidAccountOperationException {
        if (!isActive) throw new InvalidAccountOperationException("Account closed.");
        if (amount < 0) throw new NegativeDepositException("Deposit amount must be positive.");
        balance += amount;
...
class SecureBankAccount extends BankAccountDecorator {
    ...
    public void withdraw(double amount) throws OverdrawException, InvalidAccountOperationException {
        if (amount > 500) {
            throw new InvalidAccountOperationException("Withdrawal limit exceeded: maximum $500");
        }
